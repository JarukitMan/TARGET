...
def gen_crosswalk_route(routes, crosswalk_info):
    ...
    for route in routes:
        meet_crosswalk, intersection_crosswalk = get_crosswalk_info(
            route.start_waypoint, route.end_waypoint, crosswalk_info
        )
...
def find_predecessor_route(routes, query_lane):
    ...
    for route in routes:
        if (
            route.end_waypoint.road_id == query_lane[0]
            and route.end_waypoint.lane_id == query_lane[1]
        ):
...
def get_route_direction(start_waypoint, end_waypoint):
    if (
        abs(start_waypoint.transform.location.x - end_waypoint.transform.location.x)
        <= 1
        or abs(start_waypoint.transform.location.y - end_waypoint.transform.location.y)
        <= 1
    ):
        direction = 0
    else:
        ...
        if diff > 0 - 2 and diff < 180 - 2:
            direction = -1  # right
        elif (diff >= 180 - 2 and diff <= 180 + 2) or (
            diff >= -180 - 2 and diff <= -180 + 2
        ):
...
def get_predecessor_route(waypoint, routes):
    ...
    for route in routes:
        if (
            route.end_waypoint.transform.location.x == waypoint.transform.location.x
            and route.end_waypoint.transform.location.y == waypoint.transform.location.y
            and route.end_waypoint.transform.location.z == waypoint.transform.location.z
        ):
            target_route = route
            break
...
# NOTE: Seems like a TODO function?
...
class Route:
    def __init__(self, id, start_waypoint, end_waypoint, map_info):
        ...
        self.length = math.sqrt(
            (
                ...
                - self.end_waypoint.transform.location.x
            )
            ** 2
            + (
                self.start_waypoint.transform.location.y
                - self.end_waypoint.transform.location.y
            )
            ** 2
        )
    ...
    def is_no_turn(self, map_info, routes):
        ...
        for obj in opendrive_road.objects:
            if (
                "noturnleft" in obj["name"].lower()
                and self.start_lane == -3
                and self.is_intersection(routes)
            ):
    ...
    def is_t_intersection(self, routes):
        ...
        for route in routes:
            ...
            if (
                route.id != self.id
                and intersect
                and intersect_angle > 85
                and intersect_angle < 100
            ):
                ...
                for another_route in routes:
                    ...
                    if (
                        abs(
                            another_route.start_waypoint.transform.location.x
                            - self.start_waypoint.transform.location.x
                        )
                        <= 1
                        and abs(
                            another_route.start_waypoint.transform.location.y
                            - self.start_waypoint.transform.location.y
                        )
                        <= 1
                        and another_route.direction == 0
                    ):
                        flag = True
                        break
                if not flag:
    ...
    def get_road_mark(self):
        return (
            self.start_waypoint.left_lane_marking.type,
            self.start_waypoint.right_lane_marking.type,
        )
    ...
    def get_cross_routes(self, routes, direction=0, position=0, num=1):
        ...
        for route in routes:
            ...
            if (
                route.id != self.id
                and intersect
                and route.start_waypoint.transform.location
                != self.end_waypoint.transform.location
                and route.direction == direction
                and get_route_direction(self.start_waypoint, route.start_waypoint)
                == relative_position
            ):
                target_routes.append(route)
    ...
    def get_predecessor_route(self, routes):
        ...
        for route in routes:
            if (
                route.end_waypoint.transform.location.x
                == self.start_waypoint.transform.location.x
                and route.end_waypoint.transform.location.y
                == self.start_waypoint.transform.location.y
                and route.end_waypoint.transform.location.z
                == self.start_waypoint.transform.location.z
            ):
                target_route = route
                break
        return target_route
    ...
    def get_successor_route(self, routes):
        ...
        for route in routes:
            if (
                abs(
                    route.start_waypoint.transform.location.x
                    - self.end_waypoint.transform.location.x
                )
                <= 2
                and abs(
                    route.start_waypoint.transform.location.y
                    - self.end_waypoint.transform.location.y
                )
                <= 2
                and abs(
                    route.start_waypoint.transform.location.z
                    - self.end_waypoint.transform.location.z
                )
                <= 2
            ):
                target_route = route
